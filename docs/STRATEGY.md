...
| Tool plugin hooks (before/after_tool_call, agent_end) + auto-compaction | **have** | ToolExecutor exposes before/after hooks, and history beyond the recent window is deterministically compacted into artifacts, evidence, failures, and attempted branches. The compactor avoids another model call and preserves the state needed to prevent repeated exploration. | — |
...
| MCP client (stdio + streamable-HTTP, OAuth/mTLS, toolFilter globs) | **partial** | Browser MCP plus a generic, namespaced stdio provider now route allowlisted Sibyl/Argus tools through the executor and capability envelope. Still missing: streamable HTTP, auth, manifest-level filter globs, and an isolated generic gateway with destination enforcement. | M |
