...
app/
  ...
  provider_gateway shared runtime for isolated browser/email/calendar/vision services
...
Worker mode mints short-lived Ed25519 authority grants. The dedicated email,
calendar, vision, and browser gateways plus the egress proxy receive only the public
verifier, independently enforce each grant, and return per-run audit events stored in
the task and Receipt. Network capabilities require explicit destination hosts; empty
